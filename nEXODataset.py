...
def test(testloader, epoch, pitch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint_%dmm_cl' % pitch):
            os.mkdir('checkpoint_%dmm_cl' % pitch)
        torch.save(state, './checkpoint_%dmm_cl/ckpt.t7' % pitch)
        best_acc = acc
...
if __name__ == "__main__":
    ...
    pickle.dump( pickle_dump, open( "save_%dmm_cl.p" % args.pitch, "wb" ) )
